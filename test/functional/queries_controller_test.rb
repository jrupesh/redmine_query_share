...
class QueriesControllerTest < ActionController::TestCase
  fixtures :projects, :users, :members, :member_roles, :roles, :trackers, :issue_statuses, :issue_categories,
    :enumerations, :issues, :custom_fields, :custom_values, :queries, :enabled_modules, :email_addresses
  ...
  def test_new_project_query
    ...
    assert_select 'input[name=?][value="3"]', 'query[visibility]'
  ...
  def test_create_project_group_query
    @request.session[:user_id] = 2
    post :create,
         :project_id => 'ecookbook',
         :default_columns => '1',
  ...
  def test_create_user_group_query_with_custom_columns
    @request.session[:user_id] = 3
    post :create,
         :fields => ["status_id", "assigned_to_id"],
         :operators => {"assigned_to_id" => "=", "status_id" => "o"},
  ...
  def editUserGroupQuery
    query = Query.find_by_id(3)
    query.visibility = 3
    query.users << User.find(2)
    query.users << User.find(4)
    query.save
  end

  def test_edit_global_user_group_query
    editUserGroupQuery
    @request.session[:user_id] = 3
    get :edit, :id => 3
    assert_response :success
    assert_template 'edit'
    assert_select 'input[name=?]', 'query[visibility]', 0
    assert_select 'input[name=?][value="3"][checked=checked]', 'query[visibility]'
    assert_select 'input[name=query_is_for_all][type=checkbox][checked=checked][disabled=disabled]'
  end
  ...
  def test_udpate_user_group_private_query
    @request.session[:user_id] = 3
    put :update,
         :id => 3,
         :default_columns => '1',
  ...
  def test_update_user_group_public_query
    @request.session[:user_id] = 1
    put :update,
         :id => 4,
         :default_columns => '1',
